...
`;
    ...
    const reviewContent = (): void => {
        editor.current?.showAIAssistantPopup();
        editor.current?.executeAIPrompt('Proof read the editor content.');
    };
    return (
        <div>
            <RichTextEditorComponent ref={editor} toolbarSettings={toolbarSettings} aiAssistantSettings={aiAssistantSettings} aiAssistantPromptRequest={onPromptRequest}>
            ...
            <br />
            <button className="e-btn e-primary" onClick={reviewContent}><span className="e-icons e-btn-icon e-check-large"></span>Proof read</button>
        </div>
    );
}
