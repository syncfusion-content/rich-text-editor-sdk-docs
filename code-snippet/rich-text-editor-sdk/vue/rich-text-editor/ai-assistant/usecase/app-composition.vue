...
import { RichTextEditorComponent as EjsRichtexteditor, Toolbar, Link, Image, HtmlEditor, AIAssistant } from "@syncfusion/ej2-vue-richtexteditor";
...
Company Name
`;
    rteInstance.value.addAIPromptResponse(aiResponse, false);
    rteInstance.value.addAIPromptResponse(aiResponse, true);
  }, 300);
};
const reviewContent = () => {
  rteInstance.value.showAIAssistantPopup();
  rteInstance.value.executeAIPrompt('Proof read the editor content.');
};

const richtexteditor = [Toolbar, Link, Image, HtmlEditor, AIAssistant];
