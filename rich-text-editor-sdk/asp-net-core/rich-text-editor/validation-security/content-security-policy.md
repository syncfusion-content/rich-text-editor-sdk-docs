...
The following example shows a recommended Content Security Policy configuration for an ASP.NET Core application that hosts the Syncfusion Rich Text Editor. Configure the policy in `Startup.cs` (or `Program.cs` for .NET 6+) by adding the `Content-Security-Policy` response header in middleware.
