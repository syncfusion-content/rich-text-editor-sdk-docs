...
description: Learn how to configure toolbar items, layout, and floating behavior, add custom toolbar items, and sync an external UI with toolbar events in the JavaScript Modern Rich Text Editor.
...
If you don't want a toolbar at all — for example, when the editor is driven entirely by your own UI, as in the [ribbon sample](#updatedtoolbarstatus-event) further down this page — set `toolbarSettings.enable` to `false` and no toolbar is rendered; the editor stays fully usable programmatically.
...
Beyond these, you can add your own entries — see [Custom toolbar item](#custom-toolbar-item) below.
