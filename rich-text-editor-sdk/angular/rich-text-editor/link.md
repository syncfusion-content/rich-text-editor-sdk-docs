---
layout: post
title: Links in Angular Rich Text Editor | Syncfusion
description: Learn how to add, edit, remove, and manage hyperlinks in the Angular Rich Text Editor for better content navigation and user experiences.
...
# Links in Angular Rich Text Editor
...
>To use the image and link tools, configure `ImageService` and `LinkService` in the provider section.

| Options | Description |
|----------------|--------------------------------------|
| Web Address | Enter or paste the destination URL for your link (e.g., `https://google.com`). |
| Display Text | Enter or edit the text you want to display for the link. |
| Tooltip | To display additional helpful information when you hover over the hyperlink, type the required text in the **Tooltip** field. |
| Open Link in New Window | Specify whether the link opens in a new window (`target="_blank"`) or not. |
...
The Rich Text Editor supports automatic link generation. When you type a URL and press <kbd>Space</kbd> or <kbd>Enter</kbd>, the editor automatically converts the typed URL into a clickable hyperlink.

## Insert a related link
...
If you want to accept the given URL without validating it and without automatically adding a protocol (such as `https://`) or domain, set the [`enableAutoUrl`](https://ej2.syncfusion.com/angular/documentation/api/rich-text-editor/index-default#enableautourl) property to `true`. By default, this property is `false`, meaning automatic URL conversion is enabled unless explicitly disabled.
