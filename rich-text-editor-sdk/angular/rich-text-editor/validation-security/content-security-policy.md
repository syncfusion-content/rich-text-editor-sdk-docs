---
layout: post
title: Content Security Policy in Angular Rich text editor | Syncfusion
description: Learn here all about Content Security Policy in Syncfusion Angular Rich text editor component of Syncfusion Essential JS 2 and more.
...
# Content Security Policy (CSP) in Angular Rich Text Editor
...
The following example shows a recommended Content Security Policy configuration for an Angular application that hosts the Syncfusion Rich Text Editor:
...
In an Angular application, the recommended way to configure a Content Security Policy is through response headers (for example, by using an `HttpInterceptor`, server-side middleware, or a hosting-provider configuration such as Nginx or a service worker) rather than the `<meta>` tag. The header-based policy is applied by the browser before any HTML is parsed and is the most secure approach.
...
> IMPORTANT
>
> When using a strict `script-src` policy, configure the Angular build to use `ng build --configuration production` with hashed/nonce-based inline script support, or initialize the editor from the component class (as shown above) so that no inline scripts are required in the HTML.
...
> NOTE
>
> For additional guidance on resolving CSP errors in Syncfusion EJ2 controls, refer to the [Resolve CSP errors in EJ2 Angular Common control](https://help.syncfusion.com/angular/common/how-to/resolve-csp-errors) documentation.
...
* [Security Considerations in EJ2 Angular Common control](https://help.syncfusion.com/angular/common/security-considerations)
* [Resolve CSP errors in EJ2 Angular Common control](https://help.syncfusion.com/angular/common/how-to/resolve-csp-errors)
