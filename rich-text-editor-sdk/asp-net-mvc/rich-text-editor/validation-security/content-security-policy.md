---
layout: post
title: Content Security Policy in ASP.NET MVC Rich text editor | Syncfusion
description: Learn here all about Content Security Policy in Syncfusion ASP.NET MVC Rich text editor control of Syncfusion Essential JS 2 and more.
...
Alternatively, apply the header through an `ActionFilterAttribute` so that it is added to every controller action:
...
> NOTE
>
> The editor initialization is performed through the Syncfusion HTML Helper, which renders the editor markup on the server. This keeps the application compliant with strict `script-src` policies because no inline JavaScript is required in the view.

In this example:

* The `Content-Security-Policy` response header (set in `Application_BeginRequest` or an `ActionFilter`) defines the Content Security Policy.
